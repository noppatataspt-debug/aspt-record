...
export default async function handler(req, res) {
  ...
  try {
    ...
    const allRecords = await fetchSubmissionRecords(SUPABASE_URL, SUPABASE_KEY, submissionId);
...
function getDfStatus(dfPercent, target) {
  ...
  if (dfNum > target) {
    return { status: 'over', label: 'เกิน Target', headerColor: '#B91C1C', bgColor: '#FEE2E2', textColor: '#7F1D1D' };
  } else if (dfNum > target * 0.75) {
    return { status: 'warning', label: 'ใกล้ Target', headerColor: '#C2410C', bgColor: '#FFEDD5', textColor: '#7C2D12' };
  } else {
    return { status: 'ok', label: 'ผ่าน Target', headerColor: '#0F6E56', bgColor: '#EAF3DE', textColor: '#173404' };
  }
}
...
function buildAlertMessage(s) {
  ...
  if (validMentions.length > 0) {
    ...
    validMentions.forEach((m, idx) => {
      ...
      mentionees.push({ index: startIndex, length: tag.length, userId: m.userId });
      mentionText += tag + (idx < validMentions.length - 1 ? ' ' : '');
    });
    text += mentionText;
    text += '\nโปรดตรวจสอบกระบวนการผลิตด่วน';
    return { type: 'text', text: text, mention: { mentionees: mentionees } };
  } else {
  ...
  return {
    ...
    contents: [
      { type: 'text', text: label, size: 'xxs', color: labelColor, wrap: true },
      {
        ...
        contents: [
          { type: 'text', text: String(value), size: 'md', weight: 'bold', color: valueColor, flex: 0, adjustMode: 'shrink-to-fit' },
          { type: 'text', text: ` ${unit}`, size: 'xxs', color: valueColor, flex: 0 }
        ]
      }
    ]
  };
...
function dfBanner(s, status) {
  ...
  return {
    ...
    backgroundColor: status.bgColor, cornerRadius: '8px', paddingAll: '14px', margin: 'sm',
    contents: [
      {
        type: 'box', layout: 'horizontal',
        contents: [
          { type: 'text', text: '%DF', size: 'sm', weight: 'bold', color: status.textColor, gravity: 'center', flex: 0 },
          { type: 'text', text: `${s.dfPercent}%`, size: 'xxl', weight: 'bold', color: status.textColor, align: 'end', gravity: 'center', adjustMode: 'shrink-to-fit' }
        ]
      },
      { type: 'text', text: subText, size: 'xs', color: status.textColor, align: 'end', margin: 'sm' }
    ]
  };
}
...
function dfRow(name, qty, isEven) {
  return {
    type: 'box', layout: 'horizontal',
    paddingAll: '8px', paddingStart: '10px', paddingEnd: '10px',
    backgroundColor: isEven ? '#F5F5F5' : '#FFFFFF', cornerRadius: '4px',
    contents: [
      { type: 'text', text: String(name), size: 'sm', color: '#444444', flex: 5, wrap: true },
      { type: 'text', text: `${qty.toLocaleString()} kg`, size: 'sm', color: '#111111', align: 'end', weight: 'bold', flex: 3 }
    ]
  };
}
...
function buildFlexMessage(s) {
  ...
  dfToShow.forEach((d, index) => dfRows.push(dfRow(d.name, d.qty, index % 2 === 0)));

  if (s.dfDetails.length > 5) {
    dfRows.push({ type: 'text', text: `และอีก ${s.dfDetails.length - 5} รายการ`, size: 'xs', color: '#888888', align: 'center', margin: 'sm' });
  }
  if (s.dfDetails.length === 0) {
    dfRows.push({ type: 'text', text: 'ไม่มีของเสีย', size: 'sm', color: '#047857', align: 'center', margin: 'sm' });
  }
  ...
  const dfPercentBanner = dfBanner(s, status);
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', backgroundColor: status.headerColor, paddingAll: '14px',
        contents: [
          { type: 'text', text: 'PRODUCTION LOG', color: '#FFFFFF', size: 'xs', weight: 'bold' },
          { type: 'text', text: 'บันทึกข้อมูลสำเร็จ', color: '#FFFFFF', size: 'md', weight: 'bold', margin: 'sm' }
        ]
      },
      body: {
        ...
        contents: [
          {
            ...
            contents: [
              { type: 'text', text: `${s.dept} • ${s.machine}`, size: 'md', weight: 'bold', color: '#111111', flex: 0 },
              { type: 'text', text: `${dateFormatted} • กะ${s.shift}`, size: 'xs', color: '#888888', flex: 1, wrap: false }
            ]
          },
          {
